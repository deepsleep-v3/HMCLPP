...
To build the HMCLPP launcher, you need to install JDK 17 (or higher). You can download it here: [Download Liberica JDK](https://bell-sw.com/pages/downloads/#jdk-25-lts).
...
### Get HMCLPP Source Code

- You can get the latest source code via [Git](https://git-scm.com/downloads):
  ```shell
  git clone https://github.com/deepsleep-v3/HMCLPP.git
  cd HMCLPP
  ```
- You can manually download a specific version of the source code from the [GitHub Release page](https://github.com/deepsleep-v3/HMCLPP/releases)
### Build HMCLPP

To build HMCLPP, switch to the root directory of the HMCLPP project and run the following command:

```shell
./gradlew clean makeExecutables
```

The built HMCLPP program files are located in the `HMCL/build/libs` subdirectory under the project root.
...
>
> Please use these features with caution, as improper use may cause HMCLPP to behave abnormally or even crash.
...
|-----------------------------|----------------------------------------------|-----------------------------------------------------------|-------------------------------------------------------------------------------------------------------------|---------------------------|
| `HMCL_JAVA_HOME`            |                                              | Specifies the Java used to launch HMCLPP                    |                                                                                                             | Only effective for exe/sh |
| `HMCL_JAVA_OPTS`            |                                              | Specifies the default JVM parameters when launching HMCLPP  |                                                                                                             | Only effective for exe/sh |
| `HMCL_FORCE_GPU`            |                                              | Specifies whether to force GPU-accelerated rendering      | `false`                                                                                                     |                           |
| `HMCL_ANIMATION_FRAME_RATE` |                                              | Specifies the animation frame rate of HMCLPP                | `60`                                                                                                        |                           |
| `HMCL_LANGUAGE`             |                                              | Specifies the default language of HMCLPP                    | Uses the system default language                                                                            |                           |
| `HMCL_UI_SCALE`             |                                              | Specifies the UI scaling for HMCLPP                         | Uses the system's current scaling                                                                           | Supports scale factor (1.5), percentage (150%), or DPI (144dpi).                          |
|                             | `-Dhmcl.dir=<path>`                          | Specifies the current data folder of HMCLPP                 | `./.hmcl`                                                                                                   |                           |
|                             | `-Dhmcl.home=<path>`                         | Specifies the user data folder of HMCLPP                    | Windows: `%APPDATA%\.hmcl`<br>Linux/BSD: `$XDG_DATA_HOME/hmcl`<br>macOS: `~Library/Application Support/hmcl` |                           |
...
| `HMCL_FONT`                 | `-Dhmcl.font.override=<font family>`         | Specifies the default font for HMCLPP                       | Uses the system default font                                                                                |                           |
|                             | `-Dhmcl.update_source.override=<url>`        | Specifies the update source for HMCLPP                      | `https://hmcl.huangyuhui.net/api/update_link`                                                               |                           |
...
|                             | `-Dhmcl.native.backend=<auto/jna/none>`      | Specifies the native backend used by HMCLPP                 | `auto`                                                                                                      |                           |
